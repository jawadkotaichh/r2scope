...
from modules.ices import ICESScaffold
...
class RODELearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.ices_scaffold = None
        self.ices_scaffold_optimiser = None
        self.ices_explorer_optimiser = None
        if self.use_ices:
            self.ices_scaffold = ICESScaffold(args)
            self.ices_scaffold_optimiser = RMSprop(
                params=self.ices_scaffold.parameters(),
                lr=args.ices_scaffold_lr,
                alpha=args.optim_alpha,
                eps=args.optim_eps,
            )
            self.ices_explorer_optimiser = RMSprop(
                params=self.mac.ices_explorer.parameters(),
                lr=args.ices_explore_lr,
                alpha=args.optim_alpha,
                eps=args.optim_eps,
            )
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        mac_out = []
        role_out = []
        agent_hidden_seq = [] if self.use_ices else None
        self.mac.init_hidden(batch.batch_size)
        for t in range(batch.max_seq_length):
            agent_outs, role_outs = self.mac.forward(batch, t=t)
            mac_out.append(agent_outs)
            if self.use_ices:
                agent_hidden_seq.append(self.mac.hidden_states.detach().view(batch.batch_size, self.n_agents, -1))
        ...
        if self.use_ices:
            agent_hidden_seq = th.stack(agent_hidden_seq, dim=1)[:, :-1]
        ...
        mask = transition_mask.expand_as(td_error)
        ...
        ices_stats = None
        if self.use_ices and self._should_train_ices(t_env):
            ices_stats = self.ices_scaffold.train_scaffold(
                batch, self.ices_scaffold_optimiser, self.args.ices_grad_clip
            )
            explorer_stats = self._train_ices_explorer(
                batch=batch,
                agent_hidden_seq=agent_hidden_seq,
                transition_mask=transition_mask,
                intrinsic_scaffolds=ices_stats["intrinsic_scaffolds"],
            )
            ices_stats.update(explorer_stats)
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            if ices_stats is not None:
                self.logger.log_stat("ices_alpha", self.mac.ices_alpha(t_env), t_env)
                self.logger.log_stat("ices_intrinsic_mean", ices_stats["intrinsic_mean"].item(), t_env)
                self.logger.log_stat("ices_intrinsic_std", ices_stats["intrinsic_std"].item(), t_env)
                self.logger.log_stat("ices_scaffold_loss", ices_stats["loss"].item(), t_env)
                self.logger.log_stat("ices_kl_mean", ices_stats["kl_mean"].item(), t_env)
                self.logger.log_stat("ices_explorer_loss", ices_stats["explorer_loss"].item(), t_env)
                self.logger.log_stat("ices_entropy", ices_stats["entropy"].item(), t_env)
                self.logger.log_stat("ices_value_loss", ices_stats["value_loss"].item(), t_env)
                self.logger.log_stat(
                    "ices_fraction_explore_actions",
                    ices_stats["fraction_explore_actions"].item(),
                    t_env,
                )
    ...
    def _should_train_ices(self, t_env):
        interval = max(1, int(getattr(self.args, "ices_train_interval", 1)))
        return self.use_ices and t_env % interval == 0

    def _train_ices_explorer(self, batch, agent_hidden_seq, transition_mask, intrinsic_scaffolds):
        roles = batch["roles"][:, :-1].squeeze(-1).long()
        role_avail_actions = batch["role_avail_actions"][:, :-1].float()
        avail_actions = batch["avail_actions"][:, :-1].float()
        actions = batch["actions"][:, :-1]
        intrinsic_targets = intrinsic_scaffolds.detach().squeeze(-1)

        explorer_mask = batch["ices_explore"][:, :-1].float().squeeze(-1)
        explorer_mask = explorer_mask * transition_mask.expand(-1, -1, self.n_agents)
        mask_denom = explorer_mask.sum()

        if mask_denom.item() == 0:
            zero = th.zeros((), device=self.device)
            return {
                "explorer_loss": zero,
                "entropy": zero,
                "value_loss": zero,
                "fraction_explore_actions": zero,
            }

        log_probs, entropy, values, _ = self.mac.ices_explorer.evaluate_actions(
            agent_hidden_seq,
            batch["state"][:, :-1],
            roles,
            role_avail_actions,
        ...
        advantages = intrinsic_targets - values.detach()

        policy_objective = log_probs * advantages.detach() + self.args.ices_beta * entropy
        policy_loss = -(policy_objective * explorer_mask).sum() / mask_denom
        value_loss = ((values - intrinsic_targets) ** 2 * explorer_mask).sum() / mask_denom
        explorer_loss = policy_loss + value_loss

        self.ices_explorer_optimiser.zero_grad()
        explorer_loss.backward()
        th.nn.utils.clip_grad_norm_(self.mac.ices_explorer.parameters(), self.args.ices_grad_clip)
        self.ices_explorer_optimiser.step()

        total_agent_steps = transition_mask.sum() * self.n_agents
        fraction_explore = explorer_mask.sum() / total_agent_steps.clamp_min(1.0)
        mean_entropy = (entropy * explorer_mask).sum() / mask_denom

        return {
            "explorer_loss": explorer_loss.detach(),
            "entropy": mean_entropy.detach(),
            "value_loss": value_loss.detach(),
            "fraction_explore_actions": fraction_explore.detach(),
        }
    ...
    def save_models(self, path):
        ...
        if self.use_ices:
            th.save(self.ices_scaffold.state_dict(), "{}/ices_scaffold.th".format(path))
            th.save(self.ices_scaffold_optimiser.state_dict(), "{}/ices_scaffold_opt.th".format(path))
            th.save(self.ices_explorer_optimiser.state_dict(), "{}/ices_explorer_opt.th".format(path))
    ...
    def load_models(self, path):
        ...
        if self.use_ices:
            scaffold_path = "{}/ices_scaffold.th".format(path)
            scaffold_opt_path = "{}/ices_scaffold_opt.th".format(path)
            explorer_opt_path = "{}/ices_explorer_opt.th".format(path)
            if os.path.exists(scaffold_path):
                self.ices_scaffold.load_state_dict(
                    th.load(scaffold_path, map_location=lambda storage, loc: storage)
                )
            if os.path.exists(scaffold_opt_path):
                self.ices_scaffold_optimiser.load_state_dict(
                    th.load(scaffold_opt_path, map_location=lambda storage, loc: storage)
                )
            if os.path.exists(explorer_opt_path):
                self.ices_explorer_optimiser.load_state_dict(
                    th.load(explorer_opt_path, map_location=lambda storage, loc: storage)
                )
